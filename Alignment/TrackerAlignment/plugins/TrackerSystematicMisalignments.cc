...
TrackerSystematicMisalignments::TrackerSystematicMisalignments(const edm::ParameterSet& cfg)
  : theAlignableTracker(0),
    ...
    m_xShift(cfg.getUntrackedParameter< double > ("xShift")),
    m_yShift(cfg.getUntrackedParameter< double > ("yShift")),
    m_zShift(cfg.getUntrackedParameter< double > ("zShift")),
    ...
    // flag for suppression of blind movements
    ...
    // subdetectors to which the misalignment is to be applied
    applyToSubdetectors_(convertToSubDetIds(cfg.getUntrackedParameter<std::vector<std::string> > ("applyToSubdetectors")))
{
  if (m_radialEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying radial ...";
  }
  if (m_telescopeEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying telescope ...";
  }
  if (m_layerRotEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying layer rotation ...";
  }
  if (m_bowingEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying bowing ...";
  }
  if (m_zExpEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying z-expansion ...";
  }
  if (m_twistEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying twist ...";
  }
  if (m_ellipticalEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying elliptical ...";
  }
  if (m_skewEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying skew ...";
  }
  if (m_sagittaEpsilon > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying sagitta ...";
  }
  if (m_xShift > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying shift in x ...";
  }
  if (m_yShift > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying shift in y ...";
  }
  if (m_zShift > -990.0){
    edm::LogPrint("MisalignedTracker") << "Applying shift in z ...";
  }

  // flag for suppression of blind movements
  if (suppressBlindMvmts)
    {
      edm::LogPrint("MisalignedTracker") << "Blind movements suppressed (TIB/TOB in z, TID/TEC in r)";
    }

  // compatibility with old (weird) z convention
  if (oldMinusZconvention)
    {
      edm::LogPrint("MisalignedTracker") << "Old z convention: dz --> -dz";
    }
  else
    {
      edm::LogPrint("MisalignedTracker") << "New z convention: dz --> dz";
    }
}
